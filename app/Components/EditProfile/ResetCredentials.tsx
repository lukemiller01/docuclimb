...
export default function ResetCredentials({ email }: UserEmail) {
  ...
  const [emailButtonDisabled, setEmailButtonDisabled] = useState(false); // Disable user from resetting multiple times
  const [emailSuccess, setEmailSuccess] = useState(false); // Displaying if a reset email was sent successfully

  const [passwordButtonDisabled, setPasswordButtonDisabled] = useState(false); // Disable user from resetting multiple times
  const [passwordSuccess, setPasswordSuccess] = useState(false); // Displaying if a password reset was sent successfully

  const router = useRouter();

  const resetEmail = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
